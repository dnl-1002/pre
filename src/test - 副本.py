...
y_hat_df.to_csv('E:\\plankton_pre\\pred_cop_2022全年预测.csv', index=False)
y_hat = y_hat.flatten()[-1500:]
y_hat[y_hat < 0] = 0
plt.plot(range(1500), Y_df['y'].values[-1500:], label='number')
plt.plot(range(1500), y_hat.flatten(), linestyle='dashed', label='Forecast')
...
# 只选测试集部分数据
trend_pred = trend_pred[-1500:]
seasonality_pred = seasonality_pred[-1500:]
exogenous_pred = exogenous_pred[-1500:]

# 绘制模块输出
plt.figure(figsize=(12, 8))
plt.plot(range(1500), trend_pred, label='Trend Component', color='red')
plt.plot(range(1500), seasonality_pred, label='Seasonality Component', color='green')
plt.plot(range(1500), exogenous_pred, label='salinity Component', color='orange')
