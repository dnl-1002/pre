...
def save_model_and_params(model,train_model_bio_i, save_dir='saved_models'):
    """
    保存模型和参数
    """
    import os
    ...
    # 直接保存整个模型对象
    model_path = os.path.join(save_dir, train_model_bio_i+'_nbeats_model.pkl')
    ...
    # 保存配置参数
    params_path = os.path.join(save_dir, train_model_bio_i+'_model_params.json')
...
def read_12h_train_tab_to_train_model_fun(datebase_conn,tab_train_12h_name,need_predict_bio_arr):
    ...
    if data_row > 18:
        ...
        for train_model_bio_i in need_predict_bio_arr:
            data = h12_all_train_data
            data['SnapTime'] = pd.to_datetime(data['SnapTime'])
