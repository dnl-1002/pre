...
class Nbeats(object):
    ...
    def create_stack(self):
        if self.include_var_dict is not None:
            x_t_n_inputs =  int(sum([len(x) for x in self.include_var_dict.values()]))

            # Correction because week_day only adds 1 no output_size theta_n_hidden = [x_t_n_inputs + x_s_n_hidden] + theta_n_hidden
    ...
    def predict(self, ts_loader, X_test=None, return_decomposition=False):
        ...
        with t.no_grad():
            for batch in iter(ts_loader):
                ...
                forecast, block_forecast, cumulative_forecast = self.model(insample_y=insample_y, insample_x_t=insample_x,
                ...
                cumulative_forecasts.append(cumulative_forecast.cpu().data.numpy())
                outsample_ys.append(batch['outsample_y'])
                outsample_masks.append(batch['outsample_mask'])

        forecasts = np.vstack(forecasts)

        #block_forecasts = np.concatenate(block_forecasts, axis=0)
        block_forecasts = np.vstack(block_forecasts)
        cumulative_forecasts = np.vstack(cumulative_forecasts)
        #print(f"block_forecast shape: {block_forecasts.shape}")
        outsample_ys = np.vstack(outsample_ys)
        outsample_masks = np.vstack(outsample_masks)

        self.model.train()
        if return_decomposition:
            return outsample_ys, forecasts, block_forecasts, cumulative_forecasts, outsample_masks
        else:
    ...
    def predict(self, ts_loader, X_test=None, return_decomposition=False):
        ...
        outsample_ys = []
        outsample_masks = []
        forecast_components = []  # 用来存储分解后的预测组件
        ...
        with t.no_grad():
            for batch in iter(ts_loader):
                ...
                # 获取预测结果的分解组件
                forecast_decomposed, forecast_components_batch = self.model.decomposed_prediction(
                    insample_y=insample_y,
                    insample_x_t=insample_x,
                    insample_mask=insample_mask,
                    outsample_x_t=outsample_x
                ...
                # 添加分解结果到 forecast_components 列表中
                forecast_components.append(forecast_components_batch)
        ...
        # 将预测分解结果堆叠在一起
        forecast_components = [np.vstack(component) for component in zip(*forecast_components)]

        self.model.train()

        if return_decomposition:
            return outsample_ys, forecasts, block_forecasts, forecast_components, outsample_masks
        else:
            return outsample_ys, forecasts, outsample_masks
    '''
